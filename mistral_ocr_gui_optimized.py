...
class MistralOCRApp(ctk.CTk):
    ...
    def setup_unified_processing_tab(self):
        ...
        formats = [
            ...
            ("📄 PDF (Solo dividir)", self.batch_vars['formats']['pdf']),
            ("🖼️ Imágenes separadas", self.batch_vars['formats']['images']),
            ("📦 JSON Estructurado (Mistral OCR 3)", self.batch_vars['formats']['json']),
    ...
    def create_batch_options_section(self, parent):
        ...
        # Row 3: Table Format and Extractions
        ctk.CTkLabel(frame, text="Formato Tablas:").grid(
            row=2, column=0, padx=5, pady=5, sticky="w"
        )
        ctk.CTkOptionMenu(
            frame, values=["html", "markdown", "none"],
            variable=self.batch_vars['table_format']
        ).grid(row=2, column=1, padx=5, pady=5, sticky="w")

        ctk.CTkCheckBox(
            frame, text="Extraer Headers",
            variable=self.batch_vars['extract_header']
        ).grid(row=2, column=2, padx=5, pady=5, sticky="w")

        ctk.CTkCheckBox(
            frame, text="Extraer Footers",
            variable=self.batch_vars['extract_footer']
        ).grid(row=2, column=3, padx=5, pady=5, sticky="w")
        
        # Performance optimization
        perf_frame = ctk.CTkFrame(frame)
        perf_frame.grid(row=3, column=0, columnspan=4, padx=5, pady=10, sticky="ew")
    ...
    def _processing_thread(self):
        ...
        try:
            ...
            config = ProcessingConfig(
                ...
                table_format=self.batch_vars['table_format'].get(),
                extract_header=self.batch_vars['extract_header'].get(),
                extract_footer=self.batch_vars['extract_footer'].get()
            )
            ...
            for file_idx, file_path in enumerate(input_files):
                ...
                if file_info:
                    ...
                    try:
                        # Inyectar configuración de división desde modal si existe
                        if hasattr(self, 'target_files_from_modal'):
                            self.file_processor.target_files_from_modal = self.target_files_from_modal
    ...
    def _process_traditional(self, files, config, output_dir):
        ...
        for i, file_info in enumerate(files):
            ...
            try:
                # Prepare kwargs with correct None handling for table_format
                process_kwargs = {
                    "table_format": None if config.table_format == "none" else config.table_format,
                    "extract_header": config.extract_header,
                    "extract_footer": config.extract_footer
                ...
                if 'json' in config.output_formats:
                    ...
                    self.ocr_client.save_json(response, path)
